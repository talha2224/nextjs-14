...
export default function WorkoutSetup({onRoundComplete, setTotalStrikes, totalStrikes}) {
  ...
  const audioFiles = [
    "/STRIKING_CHALLENGE_TUTORIAL_INTRO_B.mp3",
    "/STRIKING_CHALLENGE_SHORT_TUTORIAL_INTRO_B.mp3",
  ];

  // Inside your component or function
  const rightWristInBoundary = useRef(false);
  ...
  function processPoseLandmarks(results, audioRef) {
    ...
    // Check if the right wrist is currently in one of the boundary zones (first 5% or last 5% of the normalized range)
    const isInBoundaryZone =
      rightWrist.x <= 0.05 ||
      (rightWrist.x >= 0.95 && rightWrist.visibility > 0.5);

    if (isInBoundaryZone) {
      // If the wrist is in the boundary zone, mark it as such if it wasn't already
      if (!rightWristInBoundary.current) {
        rightWristInBoundary.current = true;
      }
    } else {
      // If the wrist is not in the boundary zone but was previously in, play the sound and reset the tracking
      if (rightWristInBoundary.current && rightWrist.visibility > 0.5) {
        playJabSound(audioRef);
        rightWristInBoundary.current = false;
    ...
    <Card className="w-full h-100vh relative items-center justify-center">
      <CardBody className="h-full p-3">
        <video
          ...
          src="https://kayyo-static-assests.s3.us-east-2.amazonaws.com/jab-rush.mp4"
          autoPlay
          loop
          muted
          playsInline
        ...
        <div className="p-4 z-30 relative">
          <p className="text-small">JAB TRAINING ROOM</p>
          <p className="jab-counter">JABS: {jabCount}</p>{" "}
